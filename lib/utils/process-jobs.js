...
module.exports = function(extraJob) {
  ...
  function lockOnTheFly() {
    ...
    debug('job.attrs: %s', JSON.stringify(job.attrs) );
    if ((job.attrs.data.resources || []).some(r => definitions[job.attrs.name].busyResources.indexOf(r) > -1)) {
      debug('resource limit hit for: [%s - %s]', job.attrs.name, job.attrs._id);
      return;
    }
    ...
    self._collection.findOneAndUpdate(criteria, update, options, (err, resp) => {
      ...
      if (resp.value) {
        ...
        (job.attrs.data.resources || []).forEach(r => definitions[job.attrs.name].busyResources.push(r)); 
        enqueueJobs(job);
        jobProcessing();
      }
  ...
  function processJobResult(err, job) {
    ...
    (job.attrs.data.resources || []).forEach(r => {
      const position = definitions[name].busyResources.indexOf(r);
      if (position > -1) {
        definitions[name].busyResources.splice(position, 1)
      }
    });
