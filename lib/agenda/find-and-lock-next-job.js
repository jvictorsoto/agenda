...
module.exports = function(jobName, definition, cb) {
  ...
  if (s.topology.connections().length === 0) {
    ...
    */
    const JOB_PROCESS_WHERE_QUERY = {
      $or: [{
        ...
        disabled: {$ne: true},
        'data.resources': {$nin: definition.busyResources}
      }, {
        name: jobName,
    ...
    */
    ...
    debug('JOB_PROCESS_WHERE_QUERY: %s', JSON.stringify(JOB_PROCESS_WHERE_QUERY));
    /**
    ...
    */
    ...
    this._collection.findOneAndUpdate(JOB_PROCESS_WHERE_QUERY, JOB_PROCESS_SET_QUERY, JOB_RETURN_QUERY, (err, result) => {
      ...
      if (!err && result.value) {
        ...
        debug('found a job available to lock, creating a new job on Agenda with id [%s] and locking resources [%s]', 
          result.value._id, job.attrs.data.resources || []);

        (job.attrs.data.resources || []).forEach(r => definition.busyResources.push(r)); 
      }
      if (err) {
